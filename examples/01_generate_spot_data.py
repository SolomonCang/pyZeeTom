...
def main() -> None:
    ...
    # Also write a JSON config equivalent of the same parameter file
    json_file_path = project_root / "input/config.json"
    print(f"Writing JSON config file: {json_file_path}")

    obs_entries = []
    for phase in phases:
        fname = (project_root / "input" / "spot_forward_obs" /
                 f"obs_phase_{phase:.3f}.lsd")
        hjd = 2450000 + phase
        obs_entries.append({
            "path": str(fname),
            "jdate": round(float(hjd), 5),
            "vr": 0.0,
            "polchannel": "V",
        })
    ...
    cfg = {
        "_comment": (
            "pyZeeTom configuration file generated by 01_generate_spot_data.py. "
            "Pass as param_file='input/config.json' to forward_tomography() or "
