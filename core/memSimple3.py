"""
Maximum Entropy Method (MEM) 图像重建模块
基于 Skilling & Bryan (1984, MNRAS 211, 111) 算法

⚠️  DEPRECATED - 此模块已废弃
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
此模块为旧版本 ZDI 流程的一部分，在当前简化流程中已不再使用。

新流程: pyzeetom/tomography.py
  - 直接基于局部线模型 (local_linemodel_basic.py)
  - 使用速度空间盘面积分 (velspace_DiskIntegrator.py)
  - 不需要迭代优化

保留原因:
  - 兼容旧版 mainFuncs.readParamsZDI
  - 算法参考
  - 潜在恢复需求

详细说明见: core/MEM_README.md
"""
...
def mem_iter(n1, n2, ntot, Img, Data, Fmodel, sig2, Resp, weights, defImg,
    ...
    """
    MEM 图像重建迭代函数 (Skilling & Bryan 1984)
    支持对图像向量不同部分应用不同的熵形式，用于同时进行磁场和亮度映射。

    参数：
        n1: 标准图像熵（正值，无上限）应用于 Img[0:n1]
        n2: 填充因子熵（正值，有上限）应用于 Img[n1:n2]
        ntot: 图像向量总元素数；正/负（磁）熵应用于 Img[n2:ntot]
        Img: 当前图像值数组 (长度 ntot)
        Data: 观测数据数组
        Fmodel: 模型数据数组（由当前 Img 计算得出）
        sig2: 数据标准差的平方 (sigma^2)
        Resp: 响应矩阵，模型对图像的导数 R(k,j) = dF(k)/dI(j)
        weights: 熵项权重数组
        defImg: 默认图像强度（标准熵）
        defIpm: 默认值（磁熵）
        ffIMax: 填充因子熵允许的最大图像值
        targetAim: 目标 chi^2 或目标熵值
        fixEntropy: 标志：0=拟合至目标 chi^2，1=拟合至目标熵

    返回：
        (entropy, chi2, test, Img, entStand, entFF, entMag)
    """

    # 最大搜索方向数（推荐 3-6，测试中 10-12 较优）
    maxDir = 10
    ...
    # 计算归一化搜索方向
    edir, nedir, gamma = searchDir(ntot, maxDir, Resp, sig2, fsupi, gradC,
                                   gradS, gradgradS)
...
def control(C0, gamma, Cmu, Smu, Caimq, L02, alphaMin):
    ...
    while (Pfinished == 0):
        ...
        while (afinished == 0):
            ...
            Cq = C0 + np.dot(Cmu,
                             xqp) + 0.5 * np.einsum('i,i,i', gamma, xqp, xqp)
            Cqp = C0 + np.dot(
                Cmu, xqp) + 0.5 * np.einsum('i,i,i', P + gamma, xqp, xqp)
            L2 = np.dot(xqp, xqp)
